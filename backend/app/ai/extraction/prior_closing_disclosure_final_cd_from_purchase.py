...
class PriorClosingDisclosureFinalCdFromPurchaseExtraction(BaseModel):
    ...
    closing_cost_line_items: list[dict[str, Any]] = Field(default_factory=list)
...
_CLOSING_COST_LINE_ITEMS_ROW: CoreSpec = (
    ("label", coerce_str),
    ("section", coerce_str),
...
def _parse_closing_cost_line_items(raw: Any) -> list[dict[str, Any]]:
    """Coerce the closing_cost_line_items rows — bare scalars + a per-row page/snippet source (LP-443 capture).
    ...
    for entry in raw:
        if not isinstance(entry, dict):
            continue
        row: dict[str, Any] = {
            name: coerce(entry.get(name)) for name, coerce in _CLOSING_COST_LINE_ITEMS_ROW
        }
        if (
            "source" not in row
        ):  # never clobber a declared 'source' data field; else keep provenance
            row["source"] = source_payload(entry)
        if any(row[name] is not None for name, _ in _CLOSING_COST_LINE_ITEMS_ROW):
            rows.append(row)
    return rows
...
def _parse_prior_closing_disclosure_final_cd_from_purchase_json(
    ...
    closing_cost_line_items = _parse_closing_cost_line_items(payload.get("closing_cost_line_items"))
    ...
    try:
        data = PriorClosingDisclosureFinalCdFromPurchaseExtraction.model_validate(
            {
                **core_payload,
                "closing_cost_line_items": closing_cost_line_items,
    ...
    status = derive_status(non_null + len(closing_cost_line_items), coercion_lost)
...
async def extract_prior_closing_disclosure_final_cd_from_purchase(
    ...
    logger.info(
        ...
        list_rows_total=len(result.data.closing_cost_line_items),
    )
    return result
