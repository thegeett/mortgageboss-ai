...
class AffiliatedBusinessDisclosureExtraction(BaseModel):
    ...
    affiliate_entries: list[dict[str, Any]] = Field(default_factory=list)
...
_AFFILIATE_ENTRIES_ROW: CoreSpec = (
    ("provider_name", coerce_str),
    ("service_type", coerce_str),
    ("nature_of_relationship", coerce_str),
    ("estimated_charge_or_range", coerce_str),
    ("source", coerce_str),
)


def _parse_affiliate_entries(raw: Any) -> list[dict[str, Any]]:
    """Coerce the affiliate_entries rows — bare scalars + a per-row page/snippet source (LP-443 capture).
    ...
    for entry in raw:
        if not isinstance(entry, dict):
            continue
        row: dict[str, Any] = {
            name: coerce(entry.get(name)) for name, coerce in _AFFILIATE_ENTRIES_ROW
        }
        if (
            "source" not in row
        ):  # never clobber a declared 'source' data field; else keep provenance
            row["source"] = source_payload(entry)
        if any(row[name] is not None for name, _ in _AFFILIATE_ENTRIES_ROW):
            rows.append(row)
    return rows
...
def _parse_affiliated_business_disclosure_json(
    ...
    affiliate_entries = _parse_affiliate_entries(payload.get("affiliate_entries"))
    ...
    try:
        data = AffiliatedBusinessDisclosureExtraction.model_validate(
            {
                **core_payload,
                "affiliate_entries": affiliate_entries,
    ...
    status = derive_status(non_null + len(affiliate_entries), coercion_lost)
...
async def extract_affiliated_business_disclosure(
    ...
    logger.info(
        ...
        list_rows_total=len(result.data.affiliate_entries),
    )
    return result
