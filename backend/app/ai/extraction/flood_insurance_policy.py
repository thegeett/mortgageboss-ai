"""Flood Insurance Policy extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/flood_insurance_policy.txt"
...
class FloodInsurancePolicyExtraction(BaseModel):
    """A flood insurance policy in the LP-39a shape: typed core + grouped catch-all.
...
class FloodInsurancePolicyExtractionResult(BaseModel):
    """A flood insurance policy extraction plus its outcome (mirrors the other extractor results)."""

    data: FloodInsurancePolicyExtraction
    ...
    @classmethod
    def failed(cls, reason: str) -> "FloodInsurancePolicyExtractionResult":
...
def _parse_flood_insurance_policy_json(text: str) -> FloodInsurancePolicyExtractionResult | None:
    ...
    return FloodInsurancePolicyExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_flood_insurance_policy(
    content: bytes, media_type: str
) -> FloodInsurancePolicyExtractionResult:
    """Extract flood insurance policy values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return FloodInsurancePolicyExtractionResult.failed("empty or unsupported document")

    system_prompt = load_prompt(_PROMPT_PATH)
    try:
        message = build_document_message(content=content, media_type=media_type)
    except ValueError:
        return FloodInsurancePolicyExtractionResult.failed("unsupported document media type")

    call = await run_extraction_completion(
        system=system_prompt,
        message=message,
        max_tokens=_MAX_TOKENS,
        log_label="flood_insurance_policy",
    )
    if call.text is None:
        return FloodInsurancePolicyExtractionResult.failed(call.failure_reason or "AI call failed")

    result = _parse_flood_insurance_policy_json(call.text)
    if result is None:
        logger.warning("flood_insurance_policy_extraction_parse_failed")  # no raw response logged
        return FloodInsurancePolicyExtractionResult.failed("could not parse extraction")
    ...
    logger.info(
        "flood_insurance_policy_extraction_done",
