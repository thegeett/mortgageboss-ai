...
_INSPECTION_RESULTS_ROW: CoreSpec = (
    ("type", coerce_str),
    ("date", coerce_date),
    ("result", coerce_str),
)
...
def _parse_inspection_results(raw: Any) -> list[dict[str, Any]]:
    """Coerce the inspection_results rows — bare scalars + a per-row page/snippet source (LP-443 capture).
    ...
    for entry in raw:
        if not isinstance(entry, dict):
            continue
        row: dict[str, Any] = {
            name: coerce(entry.get(name)) for name, coerce in _INSPECTION_RESULTS_ROW
        }
        if (
            "source" not in row
        ):  # never clobber a declared 'source' data field; else keep provenance
            row["source"] = source_payload(entry)
        if any(row[name] is not None for name, _ in _INSPECTION_RESULTS_ROW):
            rows.append(row)
    return rows
...
def _parse_building_permits_json(text: str) -> BuildingPermitsExtractionResult | None:
    ...
    inspection_results = _parse_inspection_results(payload.get("inspection_results"))
    ...
    status = derive_status(non_null + len(inspection_results), coercion_lost)
...
async def extract_building_permits(
    ...
    logger.info(
        ...
        list_rows_total=len(result.data.inspection_results),
    )
    return result
