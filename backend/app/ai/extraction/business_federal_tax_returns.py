"""Business Federal Tax Returns extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/business_federal_tax_returns.txt"
...
class BusinessFederalTaxReturnsExtraction(BaseModel):
    """A business federal tax returns in the LP-39a shape: typed core + grouped catch-all.
...
class BusinessFederalTaxReturnsExtractionResult(BaseModel):
    """A business federal tax returns extraction plus its outcome (mirrors the other extractor results)."""

    data: BusinessFederalTaxReturnsExtraction
    ...
    @classmethod
    def failed(cls, reason: str) -> "BusinessFederalTaxReturnsExtractionResult":
...
def _parse_business_federal_tax_returns_json(
    text: str,
) -> BusinessFederalTaxReturnsExtractionResult | None:
    """Defensively parse a model response into a business federal tax returns result. Never raises."""
    ...
    try:
        data = BusinessFederalTaxReturnsExtraction.model_validate(
    ...
    return BusinessFederalTaxReturnsExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_business_federal_tax_returns(
    content: bytes, media_type: str
) -> BusinessFederalTaxReturnsExtractionResult:
    """Extract business federal tax returns values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return BusinessFederalTaxReturnsExtractionResult.failed("empty or unsupported document")

    system_prompt = load_prompt(_PROMPT_PATH)
    try:
        message = build_document_message(content=content, media_type=media_type)
    except ValueError:
        return BusinessFederalTaxReturnsExtractionResult.failed("unsupported document media type")

    call = await run_extraction_completion(
        system=system_prompt,
        message=message,
        max_tokens=_MAX_TOKENS,
        log_label="business_federal_tax_returns",
    )
    if call.text is None:
        return BusinessFederalTaxReturnsExtractionResult.failed(
            call.failure_reason or "AI call failed"
        )

    result = _parse_business_federal_tax_returns_json(call.text)
    if result is None:
        logger.warning(
            "business_federal_tax_returns_extraction_parse_failed"
        )  # no raw response logged
        return BusinessFederalTaxReturnsExtractionResult.failed("could not parse extraction")
    ...
    logger.info(
        "business_federal_tax_returns_extraction_done",
