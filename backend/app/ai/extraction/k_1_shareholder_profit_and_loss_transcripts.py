...
class K1ShareholderProfitAndLossTranscriptsExtraction(BaseModel):
    ...
    transcript_line_items: list[dict[str, Any]] = Field(default_factory=list)
...
_TRANSCRIPT_LINE_ITEMS_ROW: CoreSpec = (
    ("line_code", coerce_str),
    ("description", coerce_str),
    ("amount", coerce_decimal),
)


def _parse_transcript_line_items(raw: Any) -> list[dict[str, Any]]:
    """Coerce the transcript_line_items rows — bare scalars + a per-row page/snippet source (LP-443 capture).
    ...
    for entry in raw:
        if not isinstance(entry, dict):
            continue
        row: dict[str, Any] = {
            name: coerce(entry.get(name)) for name, coerce in _TRANSCRIPT_LINE_ITEMS_ROW
        }
        if (
            "source" not in row
        ):  # never clobber a declared 'source' data field; else keep provenance
            row["source"] = source_payload(entry)
        if any(row[name] is not None for name, _ in _TRANSCRIPT_LINE_ITEMS_ROW):
            rows.append(row)
    return rows
...
def _parse_k_1_shareholder_profit_and_loss_transcripts_json(
    ...
    transcript_line_items = _parse_transcript_line_items(payload.get("transcript_line_items"))
    ...
    try:
        data = K1ShareholderProfitAndLossTranscriptsExtraction.model_validate(
            {
                **core_payload,
                "transcript_line_items": transcript_line_items,
    ...
    status = derive_status(non_null + len(transcript_line_items), coercion_lost)
...
async def extract_k_1_shareholder_profit_and_loss_transcripts(
    ...
    logger.info(
        ...
        list_rows_total=len(result.data.transcript_line_items),
    )
    return result
