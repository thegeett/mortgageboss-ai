"""Property Profile Non Subject extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/property_profile_non_subject.txt"
...
class PropertyProfileNonSubjectExtraction(BaseModel):
    """A property profile non subject in the LP-39a shape: typed core + grouped catch-all.
...
class PropertyProfileNonSubjectExtractionResult(BaseModel):
    """A property profile non subject extraction plus its outcome (mirrors the other extractor results)."""

    data: PropertyProfileNonSubjectExtraction
    ...
    @classmethod
    def failed(cls, reason: str) -> "PropertyProfileNonSubjectExtractionResult":
...
def _parse_property_profile_non_subject_json(
    text: str,
) -> PropertyProfileNonSubjectExtractionResult | None:
    """Defensively parse a model response into a property profile non subject result. Never raises."""
    ...
    try:
        data = PropertyProfileNonSubjectExtraction.model_validate(
    ...
    return PropertyProfileNonSubjectExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_property_profile_non_subject(
    content: bytes, media_type: str
) -> PropertyProfileNonSubjectExtractionResult:
    """Extract property profile non subject values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return PropertyProfileNonSubjectExtractionResult.failed("empty or unsupported document")

    system_prompt = load_prompt(_PROMPT_PATH)
    try:
        message = build_document_message(content=content, media_type=media_type)
    except ValueError:
        return PropertyProfileNonSubjectExtractionResult.failed("unsupported document media type")

    call = await run_extraction_completion(
        system=system_prompt,
        message=message,
        max_tokens=_MAX_TOKENS,
        log_label="property_profile_non_subject",
    )
    if call.text is None:
        return PropertyProfileNonSubjectExtractionResult.failed(
            call.failure_reason or "AI call failed"
        )

    result = _parse_property_profile_non_subject_json(call.text)
    if result is None:
        logger.warning(
            "property_profile_non_subject_extraction_parse_failed"
        )  # no raw response logged
        return PropertyProfileNonSubjectExtractionResult.failed("could not parse extraction")
    ...
    logger.info(
        "property_profile_non_subject_extraction_done",
