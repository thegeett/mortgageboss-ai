"""Bankruptcy Discharge extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/bankruptcy_discharge.txt"
...
class BankruptcyDischargeExtraction(BaseModel):
    """A bankruptcy discharge in the LP-39a shape: typed core + grouped catch-all.
...
class BankruptcyDischargeExtractionResult(BaseModel):
    """A bankruptcy discharge extraction plus its outcome (mirrors the other extractor results)."""

    data: BankruptcyDischargeExtraction
    ...
    @classmethod
    def failed(cls, reason: str) -> "BankruptcyDischargeExtractionResult":
...
def _parse_bankruptcy_discharge_json(text: str) -> BankruptcyDischargeExtractionResult | None:
    """Defensively parse a model response into a bankruptcy discharge result. Never raises."""
    ...
    try:
        data = BankruptcyDischargeExtraction.model_validate(
    ...
    return BankruptcyDischargeExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_bankruptcy_discharge(
    content: bytes, media_type: str
) -> BankruptcyDischargeExtractionResult:
    """Extract bankruptcy discharge values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return BankruptcyDischargeExtractionResult.failed("empty or unsupported document")

    system_prompt = load_prompt(_PROMPT_PATH)
    try:
        message = build_document_message(content=content, media_type=media_type)
    except ValueError:
        return BankruptcyDischargeExtractionResult.failed("unsupported document media type")

    call = await run_extraction_completion(
        system=system_prompt,
        message=message,
        max_tokens=_MAX_TOKENS,
        log_label="bankruptcy_discharge",
    )
    if call.text is None:
        return BankruptcyDischargeExtractionResult.failed(call.failure_reason or "AI call failed")

    result = _parse_bankruptcy_discharge_json(call.text)
    if result is None:
        logger.warning("bankruptcy_discharge_extraction_parse_failed")  # no raw response logged
        return BankruptcyDischargeExtractionResult.failed("could not parse extraction")
    ...
    logger.info(
        "bankruptcy_discharge_extraction_done",
