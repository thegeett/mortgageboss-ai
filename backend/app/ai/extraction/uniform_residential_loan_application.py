"""Uniform Residential Loan Application extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/uniform_residential_loan_application.txt"
...
class UniformResidentialLoanApplicationExtraction(BaseModel):
    """A uniform residential loan application in the LP-39a shape: typed core + grouped catch-all.
...
class UniformResidentialLoanApplicationExtractionResult(BaseModel):
    """A uniform residential loan application extraction plus its outcome (mirrors the other extractor results)."""

    data: UniformResidentialLoanApplicationExtraction
    ...
    @classmethod
    def failed(cls, reason: str) -> "UniformResidentialLoanApplicationExtractionResult":
...
def _parse_uniform_residential_loan_application_json(
    text: str,
) -> UniformResidentialLoanApplicationExtractionResult | None:
    """Defensively parse a model response into a uniform residential loan application result. Never raises."""
    ...
    try:
        data = UniformResidentialLoanApplicationExtraction.model_validate(
    ...
    return UniformResidentialLoanApplicationExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_uniform_residential_loan_application(
    content: bytes, media_type: str
) -> UniformResidentialLoanApplicationExtractionResult:
    """Extract uniform residential loan application values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return UniformResidentialLoanApplicationExtractionResult.failed(
    ...
    result = _parse_uniform_residential_loan_application_json(call.text)
    if result is None:
        logger.warning(
            "uniform_residential_loan_application_extraction_parse_failed"
    ...
    logger.info(
        "uniform_residential_loan_application_extraction_done",
