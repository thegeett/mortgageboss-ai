"""Loan Estimate extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/loan_estimate.txt"
...
class LoanEstimateExtraction(BaseModel):
    """A loan estimate in the LP-39a shape: typed core + grouped catch-all.
    ...
    rate_lock_expiration: TypedField[date] = Field(default_factory=TypedField)
    closing_cost_estimate_expiration: TypedField[date] = Field(default_factory=TypedField)
    in_5_years_total_payments: TypedField[Decimal] = Field(default_factory=TypedField)
    in_5_years_principal_paid: TypedField[Decimal] = Field(default_factory=TypedField)

    # --- Grouped catch-all — everything else -------------------------------- #
    additional_sections: list[CatchAllSection] = Field(default_factory=list)


class LoanEstimateExtractionResult(BaseModel):
    """A loan estimate extraction plus its outcome (mirrors the other extractor results)."""

    data: LoanEstimateExtraction
    ...
    @classmethod
    def failed(cls, reason: str) -> "LoanEstimateExtractionResult":
...
_CORE_SPEC: CoreSpec = (
    ...
    ("rate_lock_expiration", coerce_date),
    ("closing_cost_estimate_expiration", coerce_date),
    ("in_5_years_total_payments", coerce_decimal),
    ("in_5_years_principal_paid", coerce_decimal),
)


def _parse_loan_estimate_json(text: str) -> LoanEstimateExtractionResult | None:
    """Defensively parse a model response into a loan estimate result. Never raises."""
    ...
    try:
        data = LoanEstimateExtraction.model_validate(
    ...
    return LoanEstimateExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_loan_estimate(content: bytes, media_type: str) -> LoanEstimateExtractionResult:
    """Extract loan estimate values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return LoanEstimateExtractionResult.failed("empty or unsupported document")

    system_prompt = load_prompt(_PROMPT_PATH)
    try:
        message = build_document_message(content=content, media_type=media_type)
    except ValueError:
        return LoanEstimateExtractionResult.failed("unsupported document media type")

    call = await run_extraction_completion(
        system=system_prompt,
        message=message,
        max_tokens=_MAX_TOKENS,
        log_label="loan_estimate",
    )
    if call.text is None:
        return LoanEstimateExtractionResult.failed(call.failure_reason or "AI call failed")

    result = _parse_loan_estimate_json(call.text)
    if result is None:
        logger.warning("loan_estimate_extraction_parse_failed")  # no raw response logged
        return LoanEstimateExtractionResult.failed("could not parse extraction")
