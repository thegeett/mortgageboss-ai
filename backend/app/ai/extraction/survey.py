"""Survey extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/survey.txt"
...
class SurveyExtraction(BaseModel):
    """A survey in the LP-39a shape: typed core + grouped catch-all.
...
class SurveyExtractionResult(BaseModel):
    """A survey extraction plus its outcome (mirrors the other extractor results)."""

    data: SurveyExtraction
    ...
    @classmethod
    def failed(cls, reason: str) -> "SurveyExtractionResult":
...
def _parse_survey_json(text: str) -> SurveyExtractionResult | None:
    ...
    return SurveyExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_survey(content: bytes, media_type: str) -> SurveyExtractionResult:
    """Extract survey values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return SurveyExtractionResult.failed("empty or unsupported document")

    system_prompt = load_prompt(_PROMPT_PATH)
    try:
        message = build_document_message(content=content, media_type=media_type)
    except ValueError:
        return SurveyExtractionResult.failed("unsupported document media type")
    ...
    if call.text is None:
        return SurveyExtractionResult.failed(call.failure_reason or "AI call failed")

    result = _parse_survey_json(call.text)
    if result is None:
        logger.warning("survey_extraction_parse_failed")  # no raw response logged
        return SurveyExtractionResult.failed("could not parse extraction")
