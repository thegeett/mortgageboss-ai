"""Earnest Money Receipt extraction — GENERATED from a schema spec by the LP-434 generator.
...
_PROMPT_PATH = "extraction/earnest_money_receipt.txt"
...
class EarnestMoneyReceiptExtraction(BaseModel):
    """A earnest money receipt in the LP-39a shape: typed core + grouped catch-all.
...
class EarnestMoneyReceiptExtractionResult(BaseModel):
    """A earnest money receipt extraction plus its outcome (mirrors the other extractor results)."""
    ...
    @classmethod
    def failed(cls, reason: str) -> "EarnestMoneyReceiptExtractionResult":
...
def _parse_earnest_money_receipt_json(text: str) -> EarnestMoneyReceiptExtractionResult | None:
    """Defensively parse a model response into a earnest money receipt result. Never raises."""
    ...
    try:
        data = EarnestMoneyReceiptExtraction.model_validate(
    ...
    return EarnestMoneyReceiptExtractionResult(
        data=data, status=status, confidence=confidence, reasoning=reasoning
    )


async def extract_earnest_money_receipt(
    content: bytes, media_type: str
) -> EarnestMoneyReceiptExtractionResult:
    """Extract earnest money receipt values from a document's bytes (PDF/image). Never raises.

    Mirrors the existing extractors. The bytes/base64, raw response, and extracted
    values are never logged — only metadata.
    """
    if not content or media_type.lower().strip() not in _SUPPORTED_MEDIA_TYPES:
        return EarnestMoneyReceiptExtractionResult.failed("empty or unsupported document")

    system_prompt = load_prompt(_PROMPT_PATH)
    try:
        message = build_document_message(content=content, media_type=media_type)
    except ValueError:
        return EarnestMoneyReceiptExtractionResult.failed("unsupported document media type")

    call = await run_extraction_completion(
        system=system_prompt,
        message=message,
        max_tokens=_MAX_TOKENS,
        log_label="earnest_money_receipt",
    )
    if call.text is None:
        return EarnestMoneyReceiptExtractionResult.failed(call.failure_reason or "AI call failed")

    result = _parse_earnest_money_receipt_json(call.text)
    if result is None:
        logger.warning("earnest_money_receipt_extraction_parse_failed")  # no raw response logged
        return EarnestMoneyReceiptExtractionResult.failed("could not parse extraction")
    ...
    logger.info(
        "earnest_money_receipt_extraction_done",
