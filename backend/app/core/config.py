...
class Settings(BaseSettings):
    ...
    # Needs coverage flagging (LP-631/632/633): a pass FLAGS an AI-proposed need whose guideline
    # precondition the file already answers — e.g. a liability the credit report lists at the stated
    # payment, which B3-6-01 says needs no separate documentation. It never closes a need (ADR-388);
    # the processor disposes. Gated so it can be turned off without a redeploy.
    #
    # Covers BOTH sources of the flag: the deterministic predicates AND LP-633's AI retraction. They
    ...
    #
    # LP-827 — VALIDATED WHERE IT IS READ, in `services.upload_links.usable_link_origin`, and NOT by
    # a validator here. A `Settings` validator refuses to build the object, so it fires in EVERY
    # process: the API, the worker, the read-only query task, and `alembic upgrade head`. That last
    # one is the trap — `scripts/deploy` runs migrations on the NEW image using the CURRENTLY
    # deployed task definition, before the apply that would set this variable. Measured: the real
    # `uv run alembic upgrade head` dies under that environment, so the deploy that introduces the
    # requirement cannot complete, and the apply that satisfies it never runs.
