"""Tests for rental agreements lease agreements extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.rental_agreements_lease_agreements import (
    RentalAgreementsLeaseAgreementsExtraction,
    RentalAgreementsLeaseAgreementsExtractionResult,
    _parse_rental_agreements_lease_agreements_json,
    extract_rental_agreements_lease_agreements,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy rental_agreements_lease_agreements"
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_rental_agreements_lease_agreements_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_rental_agreements_lease_agreements_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_rental_agreements_lease_agreements_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_rental_agreements_lease_agreements(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = RentalAgreementsLeaseAgreementsExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == RentalAgreementsLeaseAgreementsExtraction()
