"""Tests for closing disclosure extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.closing_disclosure import (
    ClosingDisclosureExtraction,
    ClosingDisclosureExtractionResult,
    _parse_closing_disclosure_json,
    extract_closing_disclosure,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy closing_disclosure"
...
FULL_PAYLOAD = {
    "typed_core": {
        ...
        "apr": _core("1234.56"),
        "finance_charge": _core("1234.56"),
        "amount_financed": _core("1234.56"),
        "total_of_payments": _core("1234.56"),
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_closing_disclosure_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_closing_disclosure_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_closing_disclosure_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_closing_disclosure(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = ClosingDisclosureExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == ClosingDisclosureExtraction()
