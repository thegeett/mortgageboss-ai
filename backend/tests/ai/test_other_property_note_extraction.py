"""Tests for other property note extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.other_property_note import (
    OtherPropertyNoteExtraction,
    OtherPropertyNoteExtractionResult,
    _parse_other_property_note_json,
    extract_other_property_note,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy other_property_note"
...
FULL_PAYLOAD = {
    "typed_core": {
        ...
        "margin_and_caps": _core("SAMPLE"),
        "change_dates_and_index": _core("SAMPLE"),
    },
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_other_property_note_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_other_property_note_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_other_property_note_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_other_property_note(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = OtherPropertyNoteExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == OtherPropertyNoteExtraction()
