"""Tests for business federal tax returns extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.business_federal_tax_returns import (
    BusinessFederalTaxReturnsExtraction,
    BusinessFederalTaxReturnsExtractionResult,
    _parse_business_federal_tax_returns_json,
    extract_business_federal_tax_returns,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy business_federal_tax_returns"
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_business_federal_tax_returns_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_business_federal_tax_returns_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_business_federal_tax_returns_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_business_federal_tax_returns(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = BusinessFederalTaxReturnsExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == BusinessFederalTaxReturnsExtraction()
