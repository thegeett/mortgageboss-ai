"""Tests for payoff statement extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.payoff_statement import (
    PayoffStatementExtraction,
    PayoffStatementExtractionResult,
    _parse_payoff_statement_json,
    extract_payoff_statement,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy payoff_statement"
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_payoff_statement_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_payoff_statement_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_payoff_statement_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_payoff_statement(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = PayoffStatementExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == PayoffStatementExtraction()
