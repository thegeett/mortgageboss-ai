"""Tests for bankruptcy discharge notice extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.bankruptcy_discharge_notice import (
    BankruptcyDischargeNoticeExtraction,
    BankruptcyDischargeNoticeExtractionResult,
    _parse_bankruptcy_discharge_notice_json,
    extract_bankruptcy_discharge_notice,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy bankruptcy_discharge_notice"
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_bankruptcy_discharge_notice_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_bankruptcy_discharge_notice_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_bankruptcy_discharge_notice_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_bankruptcy_discharge_notice(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = BankruptcyDischargeNoticeExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == BankruptcyDischargeNoticeExtraction()
