"""Tests for flood insurance extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.flood_insurance import (
    FloodInsuranceExtraction,
    FloodInsuranceExtractionResult,
    _parse_flood_insurance_json,
    extract_flood_insurance,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy flood_insurance"
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_flood_insurance_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_flood_insurance_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_flood_insurance_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_flood_insurance(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = FloodInsuranceExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == FloodInsuranceExtraction()
