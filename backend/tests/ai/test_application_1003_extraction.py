"""Tests for application 1003 extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.application_1003 import (
    Application1003Extraction,
    Application1003ExtractionResult,
    _parse_application_1003_json,
    extract_application_1003,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy application_1003"
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_application_1003_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_application_1003_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_application_1003_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_application_1003(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = Application1003ExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == Application1003Extraction()
