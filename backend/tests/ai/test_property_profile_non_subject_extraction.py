"""Tests for property profile non subject extraction (GENERATED, LP-434) — the AI wrapper is MOCKED.
...
from app.ai.extraction.property_profile_non_subject import (
    PropertyProfileNonSubjectExtraction,
    PropertyProfileNonSubjectExtractionResult,
    _parse_property_profile_non_subject_json,
    extract_property_profile_non_subject,
)
from app.models.extraction import ExtractionStatus

PDF_BYTES = b"%PDF-1.7 dummy property_profile_non_subject"
...
def test_typed_core_coerced_with_source() -> None:
    d = _parse_property_profile_non_subject_json(FULL_JSON).data  # type: ignore[union-attr]
...
def test_all_null_core_is_failed() -> None:
    ...
    parsed = _parse_property_profile_non_subject_json(json.dumps(payload))
...
def test_parse_unparseable_returns_none(raw: str) -> None:
    assert _parse_property_profile_non_subject_json(raw) is None


async def test_extract_success(monkeypatch: pytest.MonkeyPatch) -> None:
    _mock_complete(monkeypatch, text=FULL_JSON)
    result = await extract_property_profile_non_subject(PDF_BYTES, "application/pdf")
...
def test_failed_factory() -> None:
    result = PropertyProfileNonSubjectExtractionResult.failed("nope")
    assert result.status == ExtractionStatus.FAILED
    assert result.data == PropertyProfileNonSubjectExtraction()
