...
def test_as6_activation_came_later_not_from_this_ticket() -> None:
    # LP-402 calibrated the tags but did not touch AS-6's activation (tag-work only). AS-6 was activated later,
    # on Priya's OWN sign-off in LP-429 (a separate ticket, a separate ruling) — so it is now live.
