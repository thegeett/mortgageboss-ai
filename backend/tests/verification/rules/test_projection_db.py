...
async def test_projection_counts_match_files(db_session: AsyncSession) -> None:
    ...
    assert result.tags.inserted == 163  # +4 assets (LP-323-AS-B) +2 ID-5 (LP-389-A) +2 stmt
...
async def test_as1_projects_priya_validated_false_with_spec(db_session: AsyncSession) -> None:
    ...
    # 34 = +10 AS-2..AS-12 specs (LP-323-AS-B); +1 OC-1 (LP-406-4); +1 AS-8 (LP-406-2b, on the LP-410 tag).
    assert with_spec == 36
