...
async def test_projection_counts_match_files(db_session: AsyncSession) -> None:
    ...
    assert (
        ...
        == 265  # bug-016 +1 (income.has_job_change — IN-7's scope predicate, via the overlay). ⚠️ TWO COUNTERS TRACK THIS ONE NUMBER AND ONLY ONE WAS BUMPED: test_fact_tags_files.py's already read 265, this one still read 264, so bug-016 left the suite red on a count that was correct in the sibling assertion. Both derive from load_desired_tags(); bump them together.  # bug-002 +1 (liab.is_payment_bearing — CR-1's materiality filter, via the overlay)  # LP-622 +1 (property.subject_rental_income_monthly — OC-3's amount, via the overlay)  # LP-616 +1 (id.address_role — ID-4's superseded-residence exclusion, via the overlay)  # LP-597 +1 (loan.ltv_basis_is_appraised — MI-1's guard on clearing)  # LP-597 +1 (liab.payoff_contradicted — DT-8's guard on its satisfied branch)  # LP-597 +4 (reserves.has_other_financed_properties + .other_financed_count + .other_financed_aggregate_upb + .other_financed_required_amount — B3-4.1-01's overlay, unblocked by LP-596's REO schedule)  # LP-576 +2 (reo.statement_matched_holder + reo.statement_billed_payment — DT-6's Apply)  # LP-575 +1 (reo.statement_liability_paid_off — DT-6's payoff scope)  # LP-573 +2 (liab.stated_is_mortgage + liab.payoff_marked — DT-8's inputs, via the overlay)  # LP-556 +1 (liab.creditor_name — added via vocabulary_extra.yaml)  # LP-551 +1 (txn.stated_liability_match — added via vocabulary_extra.yaml, not the xlsx)  # LP-519 +1 (stmt.repeated_money_in_max_total)  # LP-509-D1 +2 (ins.expiration_date + ins.policy_expired — IH-9's parsed binder date and its derived loan verdict)  # LP-498 +1 (contract.credits_warrant_review)  # LP-496a +1 (program.conforming_eligibility — PE-3's program.fha_min_investment_met was already in fact_tags.csv)  # LP-495b +2 (OC-3's and DT-7's judgment output tags)  # LP-495a +4 (reo.statement_disclosure + reo.statement_payment_coverage — ONE matcher, ADR-375;
