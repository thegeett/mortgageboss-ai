...
async def test_projection_counts_match_files(db_session: AsyncSession) -> None:
    ...
    assert (
        ...
        == 244  # LP-496a +1 (program.conforming_eligibility — PE-3's program.fha_min_investment_met was already in fact_tags.csv)  # LP-495b +2 (OC-3's and DT-7's judgment output tags)  # LP-495a +4 (reo.statement_disclosure + reo.statement_payment_coverage — ONE matcher, ADR-375;
...
async def test_as1_projects_priya_validated_false_with_spec(db_session: AsyncSession) -> None:
    ...
    assert (
        with_spec
        == 80  # LP-496a +PE-1/PE-3 (specs written, both ACTIVE)  # LP-495b +OC-3/DT-7 (specs written, both held)  # +RE-1/DT-6/LO-2 (LP-495a — all three ACTIVE, deterministic, no ratification)  # +CO-3 (LP-494, un-dropped)  # +CO-4/CO-5 (LP-494, INERT — built against real fields, held until a completed questionnaire exists)  # +PC-5/PC-8 (LP-493)  # +PR-2/PR-3/PR-4/PR-5/PR-7 (LP-492)  # +TI-1/TI-2/TI-6 (LP-491)  # +CR-5/CR-6/CR-8/CR-10 (LP-490, INERT — specs without live rules)
