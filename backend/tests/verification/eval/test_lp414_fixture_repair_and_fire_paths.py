...
async def test_lf6t3n_full_verdict_distribution_is_stable() -> None:
    ...
    # LP-511 reverted that enumeration (per_borrower evaluated NOTHING on the first real file — LP-513),
    # so IN-3 is one loan-level row again: 877 -> 876.
    ...
    #
    # On a REAL run the shape is the opposite: Stage A supplies `txn.is_money_in` and
    # `txn.apparent_category`, so a deposit or a non-creditor payment is DEFINITELY out of scope and
    # resolves not_applicable — scope-false beats data-missing. On LF-WCHG that leaves seven rows in
    # scope, and with `txn.stated_liability_match` all seven match a stated liability, so FR-5 produces
    # NOTHING there. The fifty below are the cost of a fixture that stops before the tags exist.
    assert len(results) == 926
    assert (
