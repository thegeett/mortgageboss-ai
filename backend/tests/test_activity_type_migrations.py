...
def _swap_migration_value_sets() -> dict[str, set[str]]:
    ...
    for path in sorted(_VERSIONS.glob("*.py")):
        ...
        for node in ast.walk(tree):
            # BOTH assignment forms. `_NEW_VALUES = (...)` is an `Assign`;
            # `_NEW_VALUES: tuple[str, ...] = (...)` is an `AnnAssign`, and reading
            # only the first made an annotated swap invisible — a migration that
            # rewrites the constraint correctly, unseen by the guard that checks
            # constraints get rewritten. Found when the merge migration joining the
            # LP-UI-033 and LP-643 heads was written in the annotated form.
            if isinstance(node, ast.Assign):
                ...
                continue
            if "_NEW_VALUES" not in names or value is None:
                continue
            try:
