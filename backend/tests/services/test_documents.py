...
async def test_list_documents_excludes_soft_deleted(db_session: AsyncSession) -> None:
    ...
    async def _add(name: str):
        ...
        return await create_document(
            ...
            document_id=did,
            filename=name,
            # LP-1000 — DISTINCT BYTES PER DOCUMENT, because this helper adds several to one loan
            # file and `create_document` now refuses a byte-identical duplicate there. Deriving the
            # content from the name keeps each one its own document.
            content=name.encode(),
            mime_type="application/pdf",
            size=10,
