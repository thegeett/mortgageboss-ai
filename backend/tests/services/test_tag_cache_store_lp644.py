...
# NO `pytestmark = pytest.mark.anyio` HERE, and the mark that was here is why these thirteen
# tests never ran. DB tests in this repo run under pytest-asyncio auto mode against a
# SESSION-scoped `db_session`; the anyio mark pulls anyio's MODULE-scoped `anyio_backend`
# fixture, and every test errored at setup with ScopeMismatch — collected, reported, and
# never reaching an assertion. `test_lp421_schedule_surfacing.py` records the same trap.
