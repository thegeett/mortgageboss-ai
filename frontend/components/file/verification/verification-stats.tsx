...
export function VerificationStats({
  ...
  const tiles: { label: string; value: number; tone: string }[] = [
    { label: "Findings", value: data.findings.length, tone: "text-foreground" },
    { label: "Blocking", value: red, tone: red > 0 ? "text-destructive" : "text-muted-foreground" },
    {
      label: "Warnings",
      value: yellow,
      tone: yellow > 0 ? "text-warning" : "text-muted-foreground",
    },
    {
      label: "Resolved",
      value: resolved,
      tone: resolved > 0 ? "text-success" : "text-muted-foreground",
    },
    {
      label: "Needs",
      value: needsCount,
      tone: needsCount > 0 ? "text-info" : "text-muted-foreground",
    },
  ];
