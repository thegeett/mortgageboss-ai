...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-md text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-3.5 [&_svg]:shrink-0",
  {
    variants: {
      ...
      // Ledger geometry (LP-UI-006): 28px default, 24px `sm`. `icon` is square at
      // the same heights and never below the 24x24 CSS px floor the SPEC sets for
      // an icon-only target. `lg` stays for the one-per-screen primary action.
      size: {
        default: "h-7 px-3",
        sm: "h-6 px-2 text-xs",
        lg: "h-8 px-4",
