...
| 6 | LP-809 Draft accumulation | AWAITING_REVIEW | see Log | | Migration b8d5e0a17c42. Adds `template_key`/`template_version` to communications, which the plan did not ask for — §6 requires them and no later M1 ticket has a migration. Partial unique index on (file, template kind); **LP-818's needs-less reply drafts will pass through it** and it needs revisiting there. No "Done when" clause (4th) |
