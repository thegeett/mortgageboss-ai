...
| 13b | LP-804b Attachment safety: sniffing, PDF sanitisation, rasterisation | AWAITING_REVIEW | see Log | | Migration f6c3d05b284e. **One new dependency, not four**: `pikepdf`, added only after measuring that `pymupdf.scrub()` removes NONE of the dangerous keys — pinned as a test so a future pymupdf that does lets someone delete it. `pypdfium2`/`puremagic`/`mail-parser` declined with reasons |
