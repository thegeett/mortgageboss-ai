...
| 8b | LP-811b Send (frontend): draft panel, copy, mail-client link | AWAITING_REVIEW | see Log | | **The navigation question answered itself** — the Communication tab has existed as a `TabPlaceholder` since LP-33 and is already in `lib/navigation.ts`. Placeholder replaced; route and nav unchanged. Textarea seeded by draft IDENTITY, since LP-809 regenerates on every add/remove |
