...
| 13 | LP-804a MIME parsing + attachment inventory (§H2 corpus) | PENDING | | | **SPLIT from LP-804** per rule 2.1 — 4–5 days with four new dependencies is one diff nobody reviews. This half is stdlib-only: the model, `message/rfc822` recursion, filename decoding, and the fixture corpus |
| 13b | LP-804b Attachment safety: sniffing, PDF sanitisation, rasterisation | PENDING | | | The other half, and the one that carries `pikepdf` / `puremagic` / TNEF. `pymupdf` is already a dependency and may remove the need for `pypdfium2` |
