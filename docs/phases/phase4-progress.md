...
| 8 | LP-811a Send (backend): the REQUESTED transition, threading artefacts, rate limit | AWAITING_REVIEW | see Log | | **SPLIT from LP-811** per protocol rule 2.1 — backend + frontend in one ticket is a diff too large to review. This half is the send endpoint, `request_needs_item()` (the ticket that finally stamps `requested_at`), the `Reply-To` / footer-tag / Bcc artefacts, and the rate limit |
| 8b | LP-811b Send (frontend): draft panel, copy, mail-client link | PENDING | | | The other half. `mailto:` disabled above the length threshold. Needs a home in the file view, which is a navigation decision this branch's new UI stream should settle |
