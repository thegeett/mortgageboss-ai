...
| [LP-825](../tickets/LP-825.md) | The communication timeline carries the whole file's activity; ten of eleven rows were not about communication | **REVIEWED** `f1e28920` → `b1f577ad` | 2 findings. The claim the ticket rests on — every activity about a message is `COMMUNICATION_*` and already carried by the Communication row — is FALSE for an existing type: `inbound_triage` logs `DOCUMENT_UPLOADED` ("A document arrived by email and was accepted", `inbound_attachment_id` in its detail), and the manifest carried filenames only, so an accepted document read identically to one nobody had looked at. Fixed by the ticket's own rule — a communication-shaped row, not the activity query back: the manifest now carries each attachment's disposition through service, API and panel. And `MESSAGE_ACTIVITY_TYPES`' retained claim was asserted by a tautology; provenance is checkable where semantics is not, so a new test fails when a message-pipeline module writes an activity type neither `COMMUNICATION_*` nor recorded as accounted for. Disproved: removing the ACTIVITY pill cannot 422 a client — the filter is component state |
| [LP-826](../tickets/LP-826.md) | Requesting a document says nothing about the draft it just wrote | **REVIEWED** `acf27ee5` → `e831e4bb` | 2 findings, the first MINE. LP-809's review fixed "a second click asks the borrower twice" in `_create_document_needs` and proved it with IN-1 — a rule that DECLARES a document — so the branch for a finding that declares none was never reached and had no duplicate check. Measured on ID-3, the rule LF-JR4T was reported against: two clicks, two needs items, two identical lines in the borrower's email. A surviving mutant then found a second hole (the reuse lookup ignoring the title would collapse two findings' asks onto one need), and the first test written to catch it also passed under the mutant because its FIXTURE ordering made the wrong answer and the right answer the same row. And `requestConsequence`'s zero branch claimed the draft holds something that, on PR-3, does not exist — measured, no draft at all |
...
| [LP-829](../tickets/LP-829.md) | Clicking a message on the timeline opens it — no endpoint returns one message's body today, so a SENT message's words are visible nowhere | **REVIEWED** `cf425232` → `f338a461` | 2 findings. Resolution was gated on `is_open_draft`, which also requires the BORROWER template key — so a party request (the same template under six other keys) opened in the dialog reading "Hello $borrower_first_name," signed "$processor_name". `build_timeline` has no template filter, so those drafts are on the timeline and clickable. LP-823's defect in the third place, and this module's docstring says it exists to prevent exactly that. Resolution now follows the body's ORIGIN. And "offers no way to change a message" asserted two absences and nothing else, so a dialog rendering nothing passed it. VERIFIED, not taken: dropping the `loan_file_id` check fails only the SIBLING-file test, not the cross-company one — the build's own correction was right |
| [LP-830](../tickets/LP-830.md) | A document uploaded through the secure link is never processed — `process_document.delay` is reached from `api/documents.py` alone, so neither the link path nor the inbound-accept path enqueues anything | **Specified** — borrower-facing |
...
| Ticket | What | Status |
|---|---|---|
| [LP-831](../tickets/LP-831.md) | The Communication page becomes a mailbox: a list of drafts, one modal to read or edit | **REVIEWED** — 1 finding, the largest of this run. `OutboundDraftPanel` carried "Copy message" and "Open in mail client" and this ticket took it off the page, leaving "Mark as sent" alone — and nothing in this product transmits mail. A processor could record that a borrower was emailed, move every need to REQUESTED and start LP-814's reminder clock, with no way to send the message at all. Both controls restored into the dialog, which needed three fields the detail payload did not carry. The deep-link fix was correct and had no test; there is one now, and the naive comparison fails it. NOTE: this commit also rewrote `docs/tickets/LP-828.md` under its hold — not reverted, because the content is a real correction, but it is the user's call |
...
| [LP-833](../tickets/LP-833.md) | Compose by picking documents from the 166-type catalog; the model writes the framing, on the request | **REVIEWED** — 1 finding: the compose resolved "this file's newest open draft" while holding the one it meant, the same shape LP-832's review found in `remove_need_from_draft`; `draft` is a parameter now. My first test for it asserted the OUTCOME and survived the mutant — the draft a request creates IS the newest when it composes — so it asserts the wiring instead. Ask 1 answered with numbers: the bound is 60s x 3 attempts plus backoff, and the ALB has no `idle_timeout` set, so AWS's 60s default cuts first; wrapping it is forbidden by LP-313/B1's stated reason, so the property is recorded. Disproved: the gateway error does NOT leave a duplicate — a repeat compose is idempotent, measured |
| [LP-834](../tickets/LP-834.md) | The secure link is added from inside the draft; minting expires the previous one | **REVIEWED** — 1 finding: the revoke was scoped to the FILE where its reason is scoped to the DRAFT. Measured — a link minted from the panel for `cosigner@example.com`, purpose "Co-borrower's pay stubs", was revoked by clicking Add secure link on the borrower's draft. LP-815 supports several live links per file and shipped first; LP-834 asserted one. Narrowed to this draft's own previous link, found by hashing the token in the stored URL, which also makes the button's warning true rather than understated. A surviving mutant earned a test for the file scoping on that lookup. Verified against a live DB: `readonly.communications` carries neither the new column nor `body` |
| [LP-835](../tickets/LP-835.md) | Writing to the other parties, from the same list and the same send path | **REVIEWED** — 2 findings. `public_router` has TWO routes and the exact-key set was pinned on the one with a `response_model`; `POST /upload/{{token}}` is `-> dict[str, str]` with none, so it was the less-constrained and unpinned one. Now pinned, with the acceptance as its control. Writing that control found the second: `_PDF` in the file was a hand-written literal `assess` cannot parse, so it could never be accepted — and every other test using it expects a REFUSAL, so a fixture that can only fail satisfied them all. Verified rather than taken: the deleted panel's address form is in the dialog, and the party filter is one exclusion rather than an allow-list |
...
| [LP-837](../tickets/LP-837.md) | A drafts indicator in the file header, on every page of a file, deep-linking into the modal | **REVIEWED** — 1 finding: the badge stated a number that can be a page rather than a total. `build_timeline` caps at 200 and returns a `truncated` flag that the payload carries, the client type declares, and `timeline-panel.tsx` — the component beside this one, on the same query — already renders; the indicator read `entries.length` and said "See all 200 drafts". Now `200+`, "At least 200" as the accessible name, and no number in the overflow link when it would be a limit. Verified rather than taken: the header does render on every file page, so deleting `DraftBadge` did not remove the count from the verification screen |
| [LP-838](../tickets/LP-838.md) | Every surface that lists or opens a message says when — including the popover, which was not named | **REVIEWED** — 3 findings, all in the guard rather than the formatting. The comment-stripping could HIDE real code (an unclosed `/*` in one string literal and a `*/` in a later one swallows the span — measured), and it existed for exactly one sentence, which now names the formatter instead of quoting it. The guard scanned `components/file/communication/` only, so it could not see `file-header.tsx` — where LP-837's popover lands and where this ticket MOVED the timestamp requirement, leaving it a done-when sentence with nothing enforcing it. And `messageTimeLabel` claimed "an unlabelled time is better than a wrong label" while returning "Created" |
